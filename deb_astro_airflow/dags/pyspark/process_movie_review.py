...
GCS_KEY_NAME = "Bronze/movie_review.csv"

spark = SparkSession.builder.appName("ProcessMovieReview").getOrCreate()

df = spark.read.csv(f'gs://{GCP_BUCKET}/{GCS_KEY_NAME}', inferSchema=True, header=True, sep=',')
...
result.write.parquet(f'gs://{GCP_STAGING_BUCKET}/movie_review/{date}', mode="overwrite")